...
var _argV = require("./nconf/stores/argv");
var _env = require("./nconf/stores/env");
var _file = require("./nconf/stores/file");
var _literal = require("./nconf/stores/literal");
var _memory = require("./nconf/stores/memory");
var formats = require("./nconf/formats");
var version = require("../package.json").version;
var _nconf;

var getNconf = function () {
  if (_nconf) {
    return _nconf;
  }
  //
  ...
  //
  _nconf.version = version;

  //
  // Setup all stores as lazy-loaded getters.
  //
  _nconf.__defineGetter__("Argv", function () {
    return _argV.Argv;
  });

  _nconf.__defineGetter__("Env", function () {
    return _env.Env;
  });

  _nconf.__defineGetter__("File", function () {
    return _file.File;
  });

  _nconf.__defineGetter__("Literal", function () {
    return _literal.Literal;
  });

  _nconf.__defineGetter__("Memory", function () {
    return _memory.Memory;
  });

  //
  // Expose the various components included with _nconf
  //
  _nconf.key = common.key;
  _nconf.path = common.path;
  _nconf.loadFiles = common.loadFiles;
  _nconf.loadFilesSync = common.loadFilesSync;
  _nconf.formats = formats;
  _nconf.Provider = Provider;

  return _nconf;
};

exports.getNconf = getNconf;
...
Provider.prototype.add = function (name, options, usage) {
  ...
  if (!getNconf()[common.capitalize(type)]) {
...
Provider.prototype.create = function (type, options, usage) {
  return new (getNconf()[common.capitalize(type.toLowerCase())])(
    options,
    usage
  );
};
